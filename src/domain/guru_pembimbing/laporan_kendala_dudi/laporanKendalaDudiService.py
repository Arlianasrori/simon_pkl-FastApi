...
from ...models_domain.laporan_kendala_dudi_model import LaporanKendalaDudiWithSiswa,LaporankendalaDudiBase
...
async def getLaporanKendalaById(id_laporan : int,id_guru : int,session : AsyncSession) -> LaporanKendalaDudiWithSiswa :
    findLaporan = (await session.execute(select(LaporanKendalaDudi).options(joinedload(LaporanKendalaDudi.siswa).joinedload(Siswa.dudi)).where(and_(LaporanKendalaDudi.id == id_laporan,LaporanKendalaDudi.siswa.has(Siswa.id_guru_pembimbing == id_guru))))).scalar_one_or_none()
